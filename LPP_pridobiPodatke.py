...
def zapisVsePostaje(datotekaVsePostaje):
    ...
    postaje =json.loads(r.get("http://data.lpp.si/stations/getAllStations").text)["data"];
    jjjj = json.dumps(postaje)
    f = open(datotekaVsePostaje,"w")
    f.write(jjjj)
    f.close()
...
def zapisPostajaVsiOdhodi(datotekaVsePostaje, datotekaOdhodiIzPostaj):
    ...
    print("loop")
    for x in postaje:
        nazaj = json.loads(r.get("http://data.lpp.si/timetables/getArrivalsOnStation", {"station_int_id": x["int_id"]}).text);
        if nazaj["success"]:
            arrs[x["int_id"]] = nazaj["data"];     
